...
import { Check, X, RotateCcw, Trophy, Timer, TimerOff } from "lucide-react";
import { getArabicQuiz } from "../data/arabicQuizzes";

const TIMER_SECONDS = 20;

export default function ArabicQuizPlay() {
  const { id } = useParams();
  const [quiz, setQuiz] = useState(() => getArabicQuiz(id));
  ...
  const handleTimeout = useCallback(() => { if (answered) return; setAnswered(true); setShowResult(true); setSelected(-1); }, [answered]);
  ...
  if (!quiz) return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-12">
      ...
      <Link to="/arabic/quiz" className="text-sm text-gray-900 dark:text-gray-100 underline mt-2 inline-block">العودة للاختبارات</Link>
    </div>
  );
  ...
  const handleSelect = (i) => { if (answered) return; setSelected(i); setAnswered(true); setShowResult(true); if (i === question.correct) setScore((s) => s + 1); };
  ...
  const handleRestart = () => { setQuiz(getArabicQuiz(id)); setCurrent(0); setSelected(null); setShowResult(false); setScore(0); setAnswered(false); setFinished(false); setTimeLeft(TIMER_SECONDS); setStarted(true); };
  ...
  if (finished) {
    ...
    const msg = pct === 100 ? "ممتاز!" : pct >= 80 ? "أحسنت!" : pct >= 60 ? "لا بأس!" : pct >= 40 ? "يمكنك أن تفعل أفضل." : "راجع الدرس وأعد المحاولة.";
    return (
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-12">
        <div className="text-center py-12">
          ...
          <p className="text-gray-500 dark:text-neutral-400 mb-8">{msg}</p>
          <div className="w-full bg-gray-100 dark:bg-neutral-800 rounded-full h-2 mb-8 max-w-xs mx-auto"><div className={`h-2 rounded-full ${pct >= 80 ? "bg-green-500" : pct >= 50 ? "bg-amber-500" : "bg-red-400"}`} style={{ width: `${pct}%` }} /></div>
          <div className="flex gap-3 justify-center">
            ...
            <Link to="/arabic/quiz" className="inline-flex items-center gap-2 bg-white dark:bg-neutral-900 text-gray-700 dark:text-neutral-300 border border-gray-200 dark:border-neutral-700 px-5 py-2.5 rounded-lg text-sm font-medium no-underline">اختبارات أخرى</Link>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-12">
      ...
      {timerEnabled && !answered && <div className="w-full bg-gray-100 dark:bg-neutral-800 rounded-full h-1 mb-6"><div className={`h-1 rounded-full transition-all duration-1000 ${timeLeft <= 5 ? "bg-red-400" : timeLeft <= 10 ? "bg-amber-400" : "bg-green-400"}`} style={{ width: `${timerPct}%` }} /></div>}
      ...
      <div className="grid gap-2 mb-6">
        {question.options.map((opt, i) => {
          let s = "border border-gray-200 dark:border-neutral-700 text-gray-700 dark:text-neutral-300 hover:border-gray-300 dark:hover:border-neutral-600 hover:bg-gray-50 dark:hover:bg-neutral-800";
          if (showResult) { if (i === question.correct) s = "border-green-300 dark:border-green-800 bg-green-50 dark:bg-green-950/40 text-green-900 dark:text-green-200"; else if (i === selected && i !== question.correct) s = "border-red-300 dark:border-red-800 bg-red-50 dark:bg-red-950/40 text-red-900 dark:text-red-200"; else s = "border-gray-100 dark:border-neutral-800 text-gray-400 dark:text-neutral-600"; } else if (selected === i) s = "border-gray-900 dark:border-gray-100 bg-gray-50 dark:bg-neutral-800 text-gray-900 dark:text-gray-100";
          return <button key={i} onClick={() => handleSelect(i)} disabled={answered} className={`w-full text-right p-4 rounded-xl text-sm font-medium transition-all cursor-pointer bg-white dark:bg-neutral-900 flex items-center justify-between ${s} ${answered ? "cursor-default" : ""}`}><span>{opt}</span>{showResult && i === question.correct && <Check size={18} className="text-green-600 dark:text-green-400 shrink-0" />}{showResult && i === selected && i !== question.correct && <X size={18} className="text-red-500 dark:text-red-400 shrink-0" />}</button>;
        })}
      </div>
