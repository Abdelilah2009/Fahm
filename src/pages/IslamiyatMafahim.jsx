...
import { islamiyatMafahim } from "../data/islamiyatMafahim";

export default function IslamiyatMafahim() {
  const [activeCategory, setActiveCategory] = useState(islamiyatMafahim[0]?.id);
  const category = islamiyatMafahim.find((m) => m.id === activeCategory);

  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-12">
      <div className="mb-10">
        <h1 className="text-3xl font-semibold tracking-tight text-gray-900 dark:text-gray-100">المفاهيم الشرعية</h1>
        <p className="mt-2 text-gray-500 dark:text-neutral-400">المصطلحات والمفاهيم المهمة للامتحان</p>
      </div>
      <div className="flex gap-2 mb-8 overflow-x-auto pb-2">
        {islamiyatMafahim.map((cat) => (
